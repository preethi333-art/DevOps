...
which one best
in monolithic
if one sevice have bug we will stop server entire all service are down 
in microservice we stop one server only that service is down

if multiple servers what about cost?
...
Problem: Since multiple virtual machines share the same physical resources, 
they may compete for CPU, RAM, and storage, which can reduce performance.

ARCHITECTURE & COMPONENTS:
client: it will interact with user( The Docker Client is where we give commands to Docker.)
user gives commands and it will be executed by docker client

daemon: manages the docker components(images, containers, volumes)( inside docker host we have)
The Docker Daemon (dockerd) is the main background service.
It:
Creates containers
Runs containers
Manages images
Manages networks and storage
Think of it as the manager that does the actual work.

Image: An image is a template used to create containers.
Docker Container 🚢 A container is a running instance of an image.

Docker host: where we install docker (ex: linux, windows, macos)

Registry: manages the images.( store images)


Docker Client gives commands → Docker Daemon performs the work
→ Images are used to create Containers → Registry stores Images.
...
docker version- to check dodker installed or not
systemctl status docker
systemctl start docker once we  start docker then we get server details server means docker host
if we have docker host then only we create docker images n containers



docker run -it --name netflix -p 80:81 netflix:v1
-p means publish
-d detach 
80: 81 --> host port: container port
container port depends on image
if nginx--> 80
if jenkins 8080
if nexus -> 8081
mysql --> 3306
...
----------------------------------
A web server primarily handles HTTP requests and serves static content, whereas 
an application server executes business logic and interacts with databases and other
services to generate dynamic responses
Customer
   |
   v
Receptionist --------> Web Server
   |
   v
Kitchen -------------> Application Server
   |
   v
Storage/Inventory ----> Database
...
we cannot remove a running container  docker stop c3 then docker rm c3
without stop we can docker rm --f c3

is it possible to create multiple containers through commands no may be in docker swarm

is it possible to stop/start multiple containers -yes
docker stop/start c1 c2 c3
docker stop $(docker ps)  --> stop all running containers
docker 

can we delete 100 containers?yes
docker rm $(docker ps -a)  --> remove all stopped and started containers

we cannot remove a image when its in use

remove images
docker rmi $(docker images)

is it possible to remove multiple containers -yes

is it possible to remove all multiple containers -yes
